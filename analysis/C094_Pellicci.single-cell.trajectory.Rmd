...
We apply two different methods to look for genes that change in expression along the trajectory.
...
pseudo <- testPseudotime(
  sce_pseudo,
  pseudotime = sce_pseudo$slingPseudotime_1,
  block = sce_pseudo$plate_number)
...
```{r, fig.asp = 1}
# TODO: Explain that this shows genes with any sort of association between 
#       trajectory and expression.
plotHeatmap(
  sce,
  order_columns_by = "slingPseudotime_1",
  colour_columns_by = c("stage", "tissue"),
...
write.csv(
  res,
  file.path(outdir, "C094_Pellicci.tradeSeq_results.csv"),
  row.names = TRUE,
  quote = FALSE)
```

## DE between start and end of trajectory

We perform a statistic test to check for DE between the starting point and the end stage of the trajectory.
...
# TODO: CSV of results
# TODO: Remove associationFit() results?
```
...
We can visualise the expression of the genes in any given set and in any given comparison using a 'barcode plot'.
