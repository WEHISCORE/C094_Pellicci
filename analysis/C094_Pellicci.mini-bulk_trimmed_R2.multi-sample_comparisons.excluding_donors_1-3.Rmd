...
description: "Read-based analysis excluding donors 1-3"
author:
  - name: Peter Hickey
...
**This report excludes samples coming from donors 1-3.**
...
outdir <- here("output", "DEGs", "excluding_donors_1-3")
...
We exclude all samples from donors 1-3.

```{r}
sce <- sce[, !sce$donor %in% 1:3]
...
- There is no strong `donor`/`plate_number` batch effect
...
Figure \@ref(fig:mds-rbe) is an MDS plot after removing the batch effect due to `donor`^[And therefore `plate_number` since `donor` is nested within this factor.].
We can more clearly observe the `stage`- and `tissue`-specific variation.
...
I've added in *CD4* and *KLRB1* (*CD161*) for good measure.

<aside>
PDF versions of these figures are available in [`output/DEGs/excluding_donors_1-3/violin_plots/logCPM.pdf`](../output/DEGs/excluding_donors_1-3/violin_plots/logCPM.pdf) and [`output/DEGs/excluding_donors_1-3/violin_plots/batch-corrected_logCPM.pdf`](../output/DEGs/excluding_donors_1-3/violin_plots/batch-corrected_logCPM.pdf).
</aside>
...
Interactive `r BiocStyle::Biocpkg("Glimma")` MD plots of the differential expression results are available from [`output/DEGs/excluding_donors_1-3/glimma-plots/`](../output/DEGs/excluding_donors_1-3/glimma-plots/).
...
CSV file of results available from [`output/DEGs/excluding_donors_1-3/aggregated_tech_reps.DEGs.csv.gz`](../output/DEGs/excluding_donors_1-3/aggregated_tech_reps.DEGs.csv.gz).
...
CSV files of the results are available from [`output/DEGs/excluding_donors_1-3/`](../output/DEGs/excluding_donors_1-3/).
