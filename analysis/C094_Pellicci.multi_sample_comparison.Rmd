...
**TODO: check if `sample` should be set like this instead (i.e. block on `donor` as well) ?**
**sce$sample <- factor(paste0(sce$plate_number, ".", sce$tissue, ".", sce$donor, ".", sce$post_hoc_sample_gate))**
**but based on Dan's description, we should not be interested in the difference between `donor`, thus the above is sufficient !?**
...
sample_colours <- setNames(
  Polychrome::palette36.colors(nlevels(sce$sample)),
  levels(sce$sample))
...
```{r barplot-by-plate, fig.cap = "Breakdown of experiment by plate."}
p1 <- ggcells(sce) + 
  geom_bar(
...
```{r collabels-umap, fig.cap = "UMAP plot, where each point represents a cell and is coloured according to the legend.", fig.asp = 0.8}
...
```{r collabels-barplot, fig.asp = 1 / 2, fig.cap = "Breakdown of cell labels by experimental factors."}
...
columns_to_keep <- c("plate_number", "tissue", "donor", "post_hoc_sample_gate")
...
colnames(design) <- sub("plate\\_number|tissue|post\\hoc\\sample\\gate", "", colnames(design))
...
The single-cell data are plotted per sample using abbreviated sample labels, e.g., `C.P043` means "C57BL/6 mouse on plate P043", `M.P043` means "Mpl mouse on plate P043", etc.
...
**check: do I need to compare between the actual `post_hoc_facs_gate` or segregate of cells based on `cluster/label` ???**
...
```{r collabels-barplot-by-plate, fig.asp = 1 / 2, fig.cap = "Breakdown of labels by experimental factors.", layout = "l-page"}
