...
*NOTE* for the unmerged data based on these figures:
  ...
  - the majority of cells in cluster `2`, `4`, and `6` represent the three types/statuses of cells at `P8` and can be found in both `Blood` and `Thymus` samples
  - most of the cells in `P6` and `P7` form one single cluster, i.e. cluster `3`, and they found mostly in `Thymus` sample only
  ...
  - merging or not will be decided based on if the feature above can be preserved or become even more explicit

**TODO**: also check with Pete, if row `P` of plate LCE511 are going to be excluded in preprocessing.
...
Having said that cells in cluster `5` do show expression of a few `mi.gdT_vs_Mono` marker genes (Figure \@ref(fig:test-mi-mono-and-gdT-markers)), the `mi.Mono_vs_gdT` marker genes are indeed quite specific to cluster `5`, suggesting the likelihood of them to be the `Intermediate monocytes`. 
...
**TODO**: based on the SingleR annotation and unique marker gene detection (FDR < 0.05), discuss with Dan and Stuart, we can then decide if cluster 5 be a true contaminant (likely be the `intermediate monocytes` from `donor 3`) that sneaked through the FACS gate and could considered to be removed.
