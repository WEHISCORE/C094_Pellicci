...
Except for cluster `8` (mostly.thymus.S1.S2.less.blood), both cluster `6` and `7` show number of unique markers already.
For instance, cluster `6` (pure.thymus.S1.S2) shows clear up-regulation of ACTG1, ACTB, GAPDH, MIR1244-3 and H3F3A expression compared to cluster `7` and `8`.
Cluster `7`(mostly.thymus.S1.S2.more.blood) also demonstrate increase in HLA (class I), CCL5, NKG7 expression, in which CCL5 and NKG7 expression seem to be relatively higher in `blood` samples.
This is apparent that at least cluster 6 and 7 are different clusters (and cluster 8 uniqueness could be shown in pairwise comparison.
...
**Summary**: 
Based on all of these findings, it demonstrate that there are 3 sub-stages within the `S1 S2 only subset`.
However, rather than divided by tissue (e.g. thymus.S1.S2, blood.S1.S2), these sub-stages are distinguished by marker gene expression.
In which (Figure \@ref(fig:violin-key-markers-cluster)), cluster `6` (pure.thymus.S1.S2) is characterized by unique markers (such as H3F3A, ACTG1, GAPDH, MIR1244-4, etc.).
Cluster  `7` (mostly.thymus.S1.S2.more.blood) is also featrured by uniquely higher expression of markers (e.g. NKG7, CCL5, etc.)
Although cluster `8` (mostly.thymus.S1.S2.less.blood) do not have global unique up-regulated markers, genes such as FYB1 and EPB41 can help distinguish cluster `8` cells from cluster `7` and `6`, respectively.

Furthermore, S1 and S2 cells are basically inseparable as in terms of transcriptomes, as indicated by highly overlapped distribution on the UMAP plot.
Having said so, we may find some clues about the key markers driven the S1 and S2 apart (e.g.1. STMN1 and MIR1244-4 seems to be more highly expression in S1 than S2; e.g.2. up-regulation of CCL5 and IL7R clearly associated with cluster 2 only) (Figure \@ref(fig:violin-key-markers-stage) and \@ref(fig:violin-key-markers-stage-cluster)).
...
```{r violin-key-markers-cluster, fig.cap = "Violin plot showing the expression of key markers stratified by clusters", fig.asp = 1.5, layout="l-body-outset"}
plot_grid(
...
```{r violin-key-markers-stage-cluster, fig.cap = "Violin plot showing the expression of key markers stratified by stages", fig.asp = 2.5, layout="l-body-outset"}
plot_grid(
