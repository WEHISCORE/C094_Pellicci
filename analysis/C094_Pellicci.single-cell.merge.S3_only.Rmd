...
*NOTE* for the unmerged data:
  - there is one big populations, which subdivided into five different clusters;
  - except for cells in cluster `1` are mostly from `Thymus`, cells in the remaining cluster are from mixture of `Blood` and `Thymus`;
  - plate-specific batch effect is also prominent without MNN correction
...
From the perspective of preserving the biological-feature-of-interest, after the batch correction, we can still observe `Blood` contains cells mostly at `P8` only, whilst `Thymus` contains cells at all three phases, i.e. `P6`, `P7` and `P8` (Figure \@ref(fig:umap-tissue-vs)).
...
```{r umap-tissue-vs, fig.cap = "UMAP plot of the dataset. Each point represents a cell and each panel highlights cells from a particular `tissue` when data is unmerged (left) and merged by manual merge 2 (right).", fig.asp = 3/4, layout="l-body"}
...
saveRDS(
  sce,
  here("data", "SCEs", "C094_Pellicci.single-cell.merge.S3_only.SCE.rds"),
  compress = "xz")
```

The processed *SingleCellExperiment* object is available (see [`data/SCEs/C094_Pellicci.single-cell.merge.S3_only.SCE.rds`](../data/SCEs/C094_Pellicci.single-cell.merge.S3_only.SCE.rds)).
