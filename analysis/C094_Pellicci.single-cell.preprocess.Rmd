...
```{r facs, fig.cap = "Expression of *CD4* and *CD161*, as measured by FACS, for each sample."}
# Some useful colours
...
```{r facs-stages, fig.cap = "Expression of *CD4* and *CD161*, as measured by FACS, for each sample and colouring each cell by its *post-hoc* `stage`."}
...
ggplot(aes(x = V525_50_A_CD4_BV510, y = B530_30_A_CD161_FITC), data = x) +
  ...
  scale_colour_manual(
    ...
    name = "stage") +
  theme_cowplot(font_size = 10) +
  facet_wrap(tissue ~ donor, labeller = label_both) +
...
Figure \@ref(fig:qcplot-single-cells-by-plate-number) shows that QC metrics are generally consistent across plates.
...
We observe that the `Thymus` samples generally have more UMIs, genes detected, and lower percentage of UMIs that are mapped to mitochondrial RNA, compared to the `Blood` samples.
However, this is partly because the the `Blood` samples have poorer QC metrics in `NN215` than in `NN227` whereas the `Thymus` samples have consistent QC metrics across sequencing runs `NN215` and `NN227`^[This difference in QC metrics by sequencing run by tissue is not explained by the `sample_type` (i.e. 50 cells vs. 99 or 100 cells).].
...
p1 <- plotColData(
  ...
  x = I(
    interaction(
      sce[, sce$sample_type != "Single cell"]$donor, 
      sce[, sce$sample_type != "Single cell"]$sequencing_run,
      drop = TRUE, 
      lex.order = TRUE)),
  other_fields = "tissue",
  point_size = 1,
  ...
  scale_colour_manual(values = donor_colours, name = "donor") +
  scale_shape_manual(
    values = c("50 cells" = 24, "99 cells" = 22, "100 cells" = 21))
p2 <- plotColData(
  sce[, sce$sample_type != "Single cell"],
  "detected",
  x = I(
