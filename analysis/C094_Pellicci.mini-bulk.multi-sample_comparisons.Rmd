...
This report analyses the read counts of the `Blood` and `Thymus` samples only.
The report begins with some general [Setup] and is followed by various analyses denoted by a prefix (`y`):

**TODO** Add any additional subset analyses
...
# TODO: Order by sex once available?
...
sce$rep <- dplyr::group_by(
  ...
  dplyr::pull(rep)
colnames(sce) <- interaction(sce$sample, sce$stage, sce$rep, drop = TRUE, lex.order = TRUE)
...
At this stage, no sample filtering.

```{r}
# TODO: Any replicate filtering? E.g., some samples have < 100,000 reads.
```
...
We have up to `r min(table(interaction(sce$sample, sce$stage, drop = TRUE, lex.order = TRUE)))` - `r max(table(interaction(sce$sample, sce$stage, drop = TRUE, lex.order = TRUE)))` technical replicates from each biological replicate.
...
- There is an obvious `sequencing_run`/`plate_number` batch effect
...
Dan is interested in the following genes on the basis of their protein expression in different `tissue` and `stage`.
...
```{r, fig.cap = "MDS plots of aggregated technical replicates coloured by various experimental factors."}
y_mds <- plotMDS(y, plot = FALSE)
y_df <- cbind(data.frame(x = y_mds$x, y = y_mds$y), y$samples)
rownames(y_df) <- colnames(y)
...
Figure \@ref(fig:md-y-1) - \@ref(fig:md-y-3) are MD plots in each comparison.
...
Interactive `r BiocStyle::Biocpkg("Glimma")` MD plots of the differential expression results are available from [`output/DEGs/reads/glimma-plots/`](../output/DEGs/reads/glimma-plots/).
...
```{r}
dgelrt <- glmLRT(y_dgeglm, contrast = y_contrast[, "Thymus.S3_vs_Thymus.S1"])
...
```{r}
dgelrt <- glmLRT(y_dgeglm, contrast = y_contrast[, "Thymus.S2_vs_Thymus.S1"])
tt <- topTags(dgelrt, p.value = 0.05, n = Inf)
if (nrow(tt)) {
  tt$table[, c("logFC", "logCPM", "LR", "PValue", "FDR")] %>%
    DT::datatable(caption = "DEGs (FDR < 0.05)") %>%
...
```{r}
dgelrt <- glmLRT(y_dgeglm, contrast = y_contrast[, "Thymus.S3_vs_Thymus.S2"])
tt <- topTags(dgelrt, p.value = 0.05, n = Inf)
if (nrow(tt)) {
  tt$table[, c("logFC", "logCPM", "LR", "PValue", "FDR")] %>%
    DT::datatable(caption = "DEGs (FDR < 0.05)") %>%
...
Figure \@ref(fig:heatmap-y-1) - \@ref(fig:heatmap-y-3) are heatmaps showing the expression of the DEGs in each comparison.
...
```{r heatmap-y-1, fig.asp = 1.5, fig.cap = "Heatmap of upper-quartile-normalized logCPM values for the top-50 DEGs."}
j <- "Thymus.S3_vs_Thymus.S1"
dgelrt <- glmLRT(y_dgeglm, contrast = y_contrast[, j])
tt <- topTags(dgelrt, p.value = 0.05, n = Inf)
pheatmap(
  cpm(y, log = TRUE)[head(rownames(tt), 50), ],
  cluster_rows = TRUE,
  cluster_cols = FALSE,
...
```{r heatmap-y-2, fig.asp = 1.5, fig.cap = "Heatmap of upper-quartile-normalized logCPM values for the top-50 DEGs."}
j <- "Thymus.S2_vs_Thymus.S1"
dgelrt <- glmLRT(y_dgeglm, contrast = y_contrast[, j])
tt <- topTags(dgelrt, p.value = 0.05, n = Inf)
pheatmap(
...
```{r heatmap-y-3, fig.asp = 1.5, fig.cap = "Heatmap of upper-quartile-normalized logCPM values for the top-50 DEGs."}
j <- "Thymus.S3_vs_Thymus.S2"
dgelrt <- glmLRT(y_dgeglm, contrast = y_contrast[, j])
tt <- topTags(dgelrt, p.value = 0.05, n = Inf)
pheatmap(
...
```{r}
# TODO: Really need to split up gene list according to the comparison.
y_index <- ids2indices(
  list(marker_proteins = marker_proteins$Gene), 
  rownames(y))
```

```{r}
y_fry <- lapply(colnames(y_contrast), function(j) {
  fry(
    y,
    index = y_index,
    design = y_design,
...
tmp <- do.call(rbind, y_fry)
rownames(tmp) <- colnames(y_contrast)
tmp %>%
  knitr::kable(
...
- [ ] DE between blood (P5 or is it P8) and thymus (P8)? "Thymus vs Blood: comparing Stage 3 (P8 CD4- CD161+) with Blood (P5)"
  - This comparison is really Thymus.P8 vs. Blood.P8.
  - We will effectively only use donors 4-8 for this comparison as we are discarding `Blood.1` - `Blood.3`.
  - `Blood.1` - `Blood.3` are really `P5` but Dan says that ~90% of `Blood.P5` cells are likely `Blood.P8` (remembering that (P6, P7, P8) ⊆ P5)
- [ ] Compare/aggregate 50-cells with 50-cells and 100-cells with 100-cells.
  - Look at number of technical reps if adding `sample_type` to things we aggregate over
    - Donors 1-3 are all 50 cells whereas donors 4-8 have 50 cell and 99/100 cell samples
  - Will probably have to add the 50-cell summed tech reps and 100-cell summed tech reps in order to have 1 sample per biological replicate
- [ ] Discard `Blood.1` - `Blood.3` (all `P5`) but keep `Thymus.1` - `Thymus.3` for the purposes of DE analyses
