---
title: "Multi-sample comparisons with the Pellicci gamma-delta T-cell single-cell dataset"
description: |
author:
  - name: William Ho
...
knitr::opts_chunk$set(fig.path = "C094_Pellicci.single-cell.multi-sample_comparisons_files/")
```
...
1) `thymus` cells sorted from different `stage` (`S1 (CD4+/CD161-)`, `S2 (CD4-/CD161-)`, `S3 (CD4-/CD161+)`);
2) `thymus` cells in `S3 (CD4-/CD161+)` vs `blood` cells in `S3 (CD4-/CD161+)`
...
```{r}
# remove cells labelled as `Unknown` (as Dan suggested) to facilitate DE analysis
sce <- sce[,sce$stage !="Unknown"]
colData(sce) <- droplevels(colData(sce))
# Re-level to set the baseline.
sce$stage <- relevel(sce$stage, "S1 (CD4+/CD161-)")
...
**sce$sample <- factor(paste0(sce$plate_number, ".", sce$tissue, ".", sce$stage))**


**or based on Dan's target, I should group `tissue` and `stage` to get `sample` like `Thymus.S1 (CD4+/CD161-)`**

```{r}
# Define what constitutes a sample (i.e. replicate) in this experiment.
sce$rep <- paste0(sce$sample, ".", sce$stage)
...
**TODO: which value should be put in `design`?? `stage` or `tissue` ?**

```{r}
# Define colData columns to be kept for pseudo-bulked data.
columns_to_keep <- c("plate_number", "tissue", "donor", "stage", "group")
summed <- aggregateAcrossCells(
  sce,
  id = colData(sce)[, c("label", "rep", columns_to_keep)],
  coldata_merge = FALSE,
  use.dimred = FALSE,
  use.altexps = FALSE)

colnames(summed) <- paste0(summed$label, ".", summed$rep)
...
assay(summed, "correctedLogCPM") <- limma::removeBatchEffect(
  ...
  design = model.matrix(~stage, colData(summed)))
```
...
**TODO: check with Pete, if I setting this up right, i.e. block on `plate_number`, then test on variation of `tissue` and of `stage`**

```{r}
# # Pulling out a sample-level 'targets' data.frame.
# targets <- colData(summed)[!duplicated(summed$sample),]
# # Constructing the design matrix.
# design <- model.matrix(~plate_number + tissue + donor + stage, targets)
...
colnames(design) <- sub("plate\\_number|tissue|donor|stage|group", "", colnames(design))
rownames(design) <- targets$sample
```

Now that we have laid out the theory underlying the DE analysis, we repeat this process for each of the labels to identify *`tissue-` or `stage` associated* DE in each cell type.
...
```{r}
tmp <- sce[, sce$group=="Thymus.S1 (CD4+/CD161-)" | sce$group=="Thymus.S2 (CD4-/CD161-)"]
...
# > test
# List of length 2
# names(2): Thymus.S1 (CD4+/CD161-) Thymus.S2 (CD4-/CD161-)

rownames(test$Thymus.S1 (CD4+/CD161-))[test$Thymus.S1 (CD4+/CD161-)$FDR <= 0.05]
rownames(test$Thymus.S2 (CD4-/CD161-))[test$Thymus.S2 (CD4-/CD161-)$FDR <= 0.05]
...
tmp <- sce[, sce$group=="Thymus.S2 (CD4-/CD161-)" | sce$group=="Thymus.S3 (CD4-/CD161+)"]
colData(tmp) <- droplevels(colData(tmp))
# tmp$group
...
rownames(test$Thymus.S2 (CD4-/CD161-))[test$Thymus.S2 (CD4-/CD161-)$FDR <= 0.05]
rownames(test$Thymus.S3 (CD4-/CD161+))[test$Thymus.S3 (CD4-/CD161+)$FDR <= 0.05]
...
summed <- aggregateAcrossCells(
  sce,
  # columns_to_keep
  id=colData(sce)[,c("plate_number", "tissue", "donor", "stage", "group", "sample")],
  coldata.merge = FALSE,
  use.dimred = FALSE,
  use.altexps = FALSE)
...
```{r}
group <- c("groupThymus.S1 (CD4+/CD161-)", "groupThymus.S3 (CD4-/CD161+)")
```
...
metadata(de_results)
# $failed
# [1] "Blood.S3 (CD4-/CD161+)"  "Thymus.S1 (CD4+/CD161-)" "Thymus.S2 (CD4-/CD161-)" "Thymus.S3 (CD4-/CD161+)"
```
...
summed <- aggregateAcrossCells(
  sce,
  # columns_to_keep
  id=colData(sce)[,c("plate_number", "tissue", "donor", "stage", "group", "sample", "cluster")],
  coldata.merge = FALSE,
  use.dimred = FALSE,
  use.altexps = FALSE)
...
```{r}
group <- c("groupThymus.S3 (CD4-/CD161+)", "groupBlood.S3 (CD4-/CD161+)")
```
...
```{r}
group <- c("Thymus.S2 (CD4-/CD161-)", "Thymus.S3 (CD4-/CD161+)")
```
...
design <- model.matrix(~plate_number + tissue + stage, targets)

```{r}
stage <- c("stageS2 (CD4-/CD161-)", "stageS3 (CD4-/CD161+)")
```
...
In this case, we are aiming to identify labels that change in abundance among the different `stage` or `tissue`.
For example, visually, `cluster_???` has a strong and consistent difference in abundance between `stage`.
...
abundances <- unclass(abundances)
abundances_df <- as.data.frame(colData(sce)) %>%
  dplyr::count(label, sample, stage) %>%
  tidyr::pivot_wider(
    names_from = label,
    id_cols = c(stage, sample), 
    values_from = n) %>%
  dplyr::arrange(stage)
knitr::kable(
  abundances_df,
...
p1 <- ggplot(as.data.frame(colData(sce)), aes(x = label, fill = stage)) + 
  geom_bar(position = position_fill(reverse = TRUE)) +
  coord_flip() +
  ylab("Frequency") +
  scale_fill_manual(values = stage_colours) +
  theme_cowplot(font_size = 5) + 
  facet_grid(~plate_number)
p2 <- ggplot(as.data.frame(colData(sce)), aes(x = label, fill = stage)) + 
  geom_bar() +
  coord_flip() +
  ylab("Number of cells") +
  scale_fill_manual(values = stage_colours) +
