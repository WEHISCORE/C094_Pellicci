...
For instance:
Cluster `9` is characterized by having the highest level of IL7R expression among the `S3 only` subset; and it is proven to be different from cells in cluster `10` and `11` by genes, say, having higher expression of LTB and IFITM3, respectively.
Besides the NPIPB gene family, the gene AC009022.1 is the global unique marker of cluster `10`, which is also featured by the higher expression of genes, say CCL5 and SYNE2, when compared to cluster `9` and `11`, respectively.
Unlike cluster `9` or `10`, cluster `11` does not have globally unique markers.
However it does have higher expression of CCL5 when compare to cluster `9`, whilst it show no up-regulation when compare to cluster `10`, cluster `10` does show genes that are statistically different in expression (i.e. SYNE2 and AC009022.1) when compare to cluster `11`.

Whilst for the central cluster, i.e. cluster 12, it does not have any marker unique marker by itself when pairwise compared to to any other peripheral cluster (9,10, or 11), it may look uninteresting at first glance, but if we consider the opposite direction of comparison, i.e. `9 vs 12`, `10 vs 12`, and `11 vs 12`, it returns number of markers up-regulated in the peripheral clusters, but not in central cluster `12`, e.g.:
cluster 9: CAPZA1, TSPAN14, and JARID2, etc.
cluster 10: *ACTG1, *MBNL1, RSF1, etc.
cluster 11: SPN, MYO9B, CCT3, etc.
Altogether, it could indeed indicate cluster `12` represent a group iof cells that have all features common to cluster 9,10 and 11; one possibility could be that the cluster `12` is a group of **progenitor cells** for the S3 cells contained in cluster 9,10 and 11.
