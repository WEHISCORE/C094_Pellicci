...
knitr::opts_chunk$set(fig.path = "C094_Pellicci.multi-sample_comparisons_files/")
```
...
1) `thymus` cells sorted from different `stage` (`P6`, `P7`, `P8`);
...
sce <- sce[,sce$stage !="P5"]
colData(sce) <- droplevels(colData(sce))
# Re-level to set the baseline.
sce$stage <- relevel(sce$stage, "P6")
...
**or based on Dan's target, I should group `tissue` and `stage` to get `sample` like `Thymus.P6`**

```{r}
# Define what constitutes a sample (i.e. replicate) in this experiment.
sce$sample <- factor(paste0(sce$plate_number, ".", sce$tissue, ".", sce$donor, ".", sce$stage))
