---
title: "Annotating Pellicci gamma-delta T-cell single-cell dataset (thymus.S1 and thymus.S2 only)"
description: |
author:
  - name: William Ho
...
knitr::opts_chunk$set(fig.path = "C094_Pellicci.single-cell.annotate.thymusS1_thymusS2_only_files/")
```

# Preparing the data

We start from the cell selected *SingleCellExperiment* object created in ['Merging cells for Pellicci gamma-delta T-cell dataset (thymus.S1 thymus.S2 only)'](C094_Pellicci.single-cell.merge.thymusS1_thymusS2_only.html).
...
**NOTE**: Based on our explorative data analyses (EDA) on the **thymus.S1 thymus.S2 only subset**, we conclude the optimal number of clusters for demonstrating the heterogeneity of the dataset, we therefore re-cluster in here.
...
After the re-clustering, there are `r nlevels(sce$cluster)` clusters for **thymus.S1 thymus.S2 only subset** of the dataset.
...
# (M1) raw unique
#
# cluster 15 (i.e. thymus.s1.s2.mix.less.s1) 
# cluster 16 (i.e. thymus.s1.s2.mix.more.s1) 
...
saveRDS(
  uniquely_up,
  here("data", "marker_genes", "thymusS1_thymusS2_only", "C094_Pellicci.uniquely_up.cluster_15_vs_16.rds"),
  compress = "xz")

dir.create(here("output", "marker_genes", "thymusS1_thymusS2_only", "uniquely_up", "cluster_15_vs_16"), recursive = TRUE)
...
#### `Cluster 15` {.panel-name}

```{r}
##########################################
# look at cluster 15 (i.e. thymus.s1.s2.mix.less.s1)
chosen <- "15"
cluster15_uniquely_up <- uniquely_up[[chosen]]

# add description for the chosen cluster-group
x <- "(thymus.s1.s2.mix.less.s1)"
...
```{r heat-uniquely-up-logExp-cluster-15, fig.cap = "Heatmap of log-expression values in each sample for the top uniquely upregulated marker genes. Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range. Ranking of CD4 and CD161/KLRB1 from top of the DGE list sorted in ascending order of FDR and their statistical significance (TRUE = FDR < 0.05) are provided in the title", fig.asp = 1.2, layout="l-body-outset"}
...
#### `Cluster 16` {.panel-name}

```{r}
##########################################
# look at cluster 16 (i.e. thymus.s1.s2.mix.more.s1)
chosen <- "16"
cluster16_uniquely_up <- uniquely_up[[chosen]]

# add description for the chosen cluster-group
x <- "(thymus.s1.s2.mix.more.s1)"
...
DGE lists of these comparisons are available in [`output/marker_genes/thymus_only/uniquely_up/cluster_15_vs_16/`](..output/marker_genes/thymus_only/uniquely_up/cluster_15_vs_16).

**Summary**: 

For the `thymus.S1 and thymus.S2 only` subset, one would expect that there are at least 2 different groups of cells, ie. thymus.s1 and thymus.s2, upon clustering.
...
But unfortunately, increasing the number of clusters cannot achieve this purpose, but only further divide cells in the mixed bulk of thymus.S1 and thymus.S2 cells.

On the whole, the best we can achieve from the single-cell samples of the `thymus.S1 and thymus.S2 only` subset could be to compare the difference between the two different clusters of thymus.S1.S2-mix, then make use of it to perhaps validate the mini-bulk outcome.
...
```{r heat-minibulk-by-cluster, fig.cap = "Heatmap of log-expression values in each sample for the marker genes identified from the mini-bulk analysis. Cells are ordered by `cluster`. Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 1, layout="l-body-outset"}
...
plotHeatmap(
  ...
  main = "Minibulk DE expression (UP and DOWN) in thymus.S1.and.S2 subset (with cells divided by clusters)",
  column_annotation_colors = list(
    cluster = cluster_colours,
    stage = stage_colours,
    tissue = tissue_colours,
...
```{r heat-minibulk-by-group, fig.cap = "Heatmap of log-expression values in each sample for the marker genes identified from the mini-bulk analysis. Cells are ordered by `group`. Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 1, layout="l-body-outset"}
...
plotHeatmap(
  ...
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers),
  main = "Minibulk DE expression (UP and DOWN) in thymus.S1.and.S2 subset (with cells divided by groups)",
  column_annotation_colors = list(
    group = group_colours,
    stage = stage_colours,
    tissue = tissue_colours,
...
```{r heat-minibulk-by-group-clustcol, fig.cap = "Heatmap of log-expression values in each sample for the marker genes identified from the mini-bulk analysis. Cells are ordered by `group` (column-clustered). Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 1, layout="l-body-outset"}
...
plotHeatmap(
  ...
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers),
  main = "Minibulk DE expression (UP and DOWN) in thymus.S1.and.S2 subset (with cells divided by groups, column-clustered)",
  column_annotation_colors = list(
    group = group_colours,
    stage = stage_colours,
    tissue = tissue_colours,
...
```{r heat-minibulk-by-cluster-up, fig.cap = "Heatmap of log-expression values in each sample for the marker genes (up-regulated) identified from the mini-bulk analysis. Cells are ordered by `cluster`. Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 0.8, layout="l-body-outset"}
...
plotHeatmap(
  ...
  # TODO: temp trick to deal with the row-colouring problem
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers_up %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers_up),
  main = "Minibulk DE expression (UP only) in thymus.S1.and.S2 subset (with cells divided by clusters)",
  column_annotation_colors = list(
    cluster = cluster_colours,
    stage = stage_colours,
    tissue = tissue_colours,
...
```{r heat-minibulk-by-group-up, fig.cap = "Heatmap of log-expression values in each sample for the marker genes (up-regulated) identified from the mini-bulk analysis. Cells are ordered by `group`. Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 0.8, layout="l-body-outset"}
...
plotHeatmap(
  ...
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers_up %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers_up),
  main = "Minibulk DE expression (UP only) in thymus.S1.and.S2 subset (with cells divided by groups)",
  column_annotation_colors = list(
    group = group_colours,
    stage = stage_colours,
    tissue = tissue_colours,
...
```{r heat-minibulk-by-group-clustcol-up, fig.cap = "Heatmap of log-expression values in each sample for the marker genes (up-regulated) identified from the mini-bulk analysis. Cells are ordered by `group` (column-clustered). Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 0.8, layout="l-body-outset"}
...
plotHeatmap(
  ...
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers_up %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers_up),
  main = "Minibulk DE expression (UP only) in thymus.S1.and.S2 subset (with cells divided by groups, column-clustered)",
  column_annotation_colors = list(
    group = group_colours,
    stage = stage_colours,
    tissue = tissue_colours,
...
```{r heat-minibulk-by-cluster-down, fig.cap = "Heatmap of log-expression values in each sample for the marker genes (down-regulated) identified from the mini-bulk analysis. Cells are ordered by `cluster`. Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 0.6, layout="l-body-outset"}
...
plotHeatmap(
  ...
  # TODO: temp trick to deal with the row-colouring problem
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers_down %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers_down),
  main = "Minibulk DE expression (DOWN only) in thymus.S1.and.S2 subset (with cells divided by clusters)",
  column_annotation_colors = list(
    cluster = cluster_colours,
    stage = stage_colours,
    tissue = tissue_colours,
...
```{r heat-minibulk-by-group-down, fig.cap = "Heatmap of log-expression values in each sample for the marker genes (down-regulated) identified from the mini-bulk analysis. Cells are ordered by `grodown`. Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 0.6, layout="l-body-outset"}
...
plotHeatmap(
  ...
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers_down %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers_down),
  main = "Minibulk DE expression (DOWN only) in thymus.S1.and.S2 subset (with cells divided by groups)",
  column_annotation_colors = list(
...
```{r heat-minibulk-by-group-clustcol-down, fig.cap = "Heatmap of log-expression values in each sample for the marker genes (down-regulated) identified from the mini-bulk analysis. Cells are ordered by `grodown` (column-clustered). Each column is a sample, each row a gene. Colours are capped at -3 and 3 to preserve dynamic range.", fig.asp = 0.6, layout="l-body-outset"}
...
plotHeatmap(
  ...
  annotation_row = data.frame(
    thymus.s2.vs.thymus.s1 = factor(ifelse(minibulk_markers_down %in% top.uniq.minibulkDEG.a, "DE", "not DE"), levels = c("DE")),
    row.names = minibulk_markers_down),
  main = "Minibulk DE expression (DOWN only) in thymus.S1.and.S2 subset (with cells divided by groups, column-clustered)",
  column_annotation_colors = list(
    grodown = group_colours,
    stage = stage_colours,
    tissue = tissue_colours,
