...
*
Among the three merge orders, i.e. *auto merge* (Table \@ref(tab:tab1)), *manual merge 1* (Table \@ref(tab:tab2)), and *manual merge 2* (Table \@ref(tab:tab3)), the *manual merge 2* lead to the least loss of biological variance from the dataset.
...
Figure \@ref(fig:batch-correction-test) shows an overview of comparisons between the UMAP project of unmerged and merged data (from left to right column: *unmerged*, *auto merge*, *manual merge 1*, and *manual merge 2*) broken down by different experimental factors.
...
To get an insight, here we break down the UMAP plot above further by `plate_number` (Figure \@ref(fig:umap-plate-number)), `post_hoc_sample_gate` (Figure \@ref(fig:umap-post-hoc-sample-gate)), and `tissue` (Figure \@ref(fig:umap-tissue)).
...
From the perspective of preserving the biological-feature-of-interest, after the batch correction, we can still observe: (i) the "shifting" of the transcriptomic stages of cells `P6` to `P7`, then `P8` (Figure \@ref(fig:umap-post-hoc-sample-gate)); (ii) `Blood` contains cells mostly at `P8` only, whilst `Thymus` contains cells at all three phases, i.e. `P6`, `P7` and `P8` (Figure \@ref(fig:umap-tissue)).
...
Further, we also investigate the usefulness of the control cell line `SKW3` included for batch correction.
...
Notably, the `SKW3` cell line also seems to affect the grouping of some of the sample cells (where cells of cell line were grouped with those from the `Thymus` cells and form a standalone cluster in Figure \@ref(fig:umap-tissue)).
To determine the identity of these `Thymus` cells and cells contained in the strayed cluster `5`, we perform cell annotation at cluster level with SingleR using the most relevant annotation reference to gamma-delta T cells- i.e. [Monaco Immune Cell Data (GSE107011)](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE107011) [@monaco_immuneRef].

## Cell type estimation: investigation of cluster `5`
...
Figures \@ref(fig:heat-mi-fine-markers) and \@ref(fig:heat-mi-fine-markers-ordered-by-cluster) demonstrates this for cells labelled as `r example_label`.
...
**TODO**: based on the SingleR annotation (or alternative investigation), decide if cluster 5 be a true contaminant (likely be the `intermediate monocytes` from `donor 3`) that sneaked through the FACS gate and could considered to be removed.

## Cluster-specific upregulated genes: investigation of cluster `1` 
...
If we zoom-in to the heatmap of the top 50 cluster-specific marker of cluster `1`, we may notice that it maybe because of the likewise expression of markers (such as `H2AFZ`, `MIR1244-2`) drive the grouping of the cells from `Thymus 2` and `Thymus 3` with cells from the control cell line in forming the cluster `1`.
...
We then re-process the cells from the remaining `sample_name`.
...
**TODO: repeating of the MNN correction may not be necessary; consider to remove/ perform MNN optimization after the removal of irrelevant cells !**
...
There are `r nlevels(sce$cluster)` clusters detected, shown on the UMAP plot Figure \@ref(fig:clusterplot-umap-CL-removed-merged) and broken down by experimental factors in Figure \@ref(fig:cluster-barplot-CL-removed-merged).
