...
sample_gate_colours <- setNames(
  palette.colors(nlevels(sce$sample_gate), "Accent"),
...
```{r barplot-highlighting-outliers, fig.cap = "Droplets removed during QC, stratified by experimental group. "}
...
Figure \@ref(fig:umap) visualizes the droplets using the UMAP co-ordinates, highlighting cells from each `plate_number` and colouring cells by `sample_name`.

```{r umap, fig.cap = "UMAP plot of the dataset. Each point represents a droplets and is coloured by `sample_name`. Each panel highlights droplets from a particular `plate_number", fig.asp = 2 / 3}
...
Figure \@ref(fig:umap) shows the small cluster(s) of cells are associated with larger library sizes.

```{r umap-libsize, fig.cap = "UMAP plot of the dataset. Each point represents a droplets and is coloured according to the legend."}
plotUMAP(
  sce,
...
Although Figures \@ref(fig:umap) - \@ref(fig:umap-libsize) are only a preliminary summaries of the data, there are few points worth highlighting:

- The biggest differences are associated with library size, which is itself largely driven by `sample_name` (namely, `Cell line` samples have larger library sizes).
- There are some plate-associated differences.

However, it is premature to draw any strong conclusions
We will seek to mitigate the library size-associated differences in downstream analyses so that we might better investigate the `sample_name`-specific differences.
