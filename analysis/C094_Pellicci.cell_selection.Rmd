...
*NOTE* for the unmerged data based on these figures:
  ...
  - the majority of cells in cluster `2`, `4`, and `6` represent the three types/statuses of cells within `P8` and can be found in all `Blood` and `Thymus` samples
  - most of the cells in `P5`, `P6` and `P7` form one single cluster, i.e. cluster `3`, and they found mostly in `Thymus` sample only
  ...
  - merging or not will be decided based on if the feature above can be preserved or even explicitized
...
Figure \@ref(fig:umap-with-fine-cluster-labels) overlays these cell-type-labels on the UMAP plot and it shows that almost all cells of the bigger group formed by cluster `2`, `3`, `4` and `6` are corresponds to `Vd2 gd T cells`.
...
We extract the identity of the markers in the reference data sets and use them to create a heatmap of their expression in the test dataset.
...
# Get the top-10 marker genes when comparing "gamma-delta T cells" and "intermediate monocytes"
top_markers_1 <- head(all_markers[["Intermediate monocytes"]]$`Vd2 gd T cells`, 10)
top_markers_2 <- head(all_markers[["Vd2 gd T cells"]]$`Intermediate monocytes`, 10)
...
This can be visualized by row-normalizing the data, as done in Figure \@ref(fig:heat-mi-fine-markers-row-normalized) and \@ref(fig:heat-mi-fine-markers-row-normalized-ordered-by-cluster).
...
```{r, results = "hide"}
dir.create(here("output", "marker_genes", "cell_selection", "SingleR"), recursive = TRUE)
...
**TODO**: based on the SingleR annotation (or alternative investigation), we can decide if cluster 5 be a true contaminant (likely be the `intermediate monocytes` from `donor 3`) that sneaked through the FACS gate and could considered to be removed.
**NOTE**: we also look into other SingleR reference (both main and fine); all other ref just label the clusters as T cells (CD4/ CD8); the only exception is `DICE`, which label cluster 5 as "NK cells", but same for 4 more clusters so it doesn't make sense ... I think we can based only on `MI` as reference, or chat with Dan to see if this cluster is interesting or needed to be removed
...
Cluster `1` shows to possess a number of genes that are specifically upregulated in that cluster compared to all other clusters ($FDR < 0.05$).
...
If we zoom-in to the heatmap above, we may notice that there is a number of likewise expression of cluster `1` specific markers (such as `H2AFZ`, `MIR1244-2`) between the cells from `Thymus 2` and `Thymus 3` with those of  the control cell line.
...
Further, we investigate the usefulness of the control cell line `SKW3` in cluster `1` for purpose of batch correction.
...
sce2$batch <- sce2$plate_number
var_fit <- modelGeneVarWithSpikes(sce2, "ERCC", block = sce2$batch)
...
mnn_out_0 <- fastMNN(
  multiBatchNorm(sce2, batch = sce2$batch),
  batch = sce2$batch,
  cos.norm = FALSE,
  d = ncol(reducedDim(sce2, "PCA")),
  auto.merge = FALSE,
  merge.order = list(list("LCE513", "LCE514", "LCE509"), list("LCE508", "LCE511", "LCE512")),
  restrict = list(sce2$sample_name == "Cell line"), 
  subset.row = hvg)

reducedDim(sce2, "corrected") <- reducedDim(mnn_out_0, "corrected")

# generate UMAP
set.seed(18191)
sce2 <- runUMAP(sce2, dimred = "PCA", name = "UMAP")
uncorrected_umap <- cbind(
  data.frame(
    x = reducedDim(sce2, "UMAP")[, 1],
    y = reducedDim(sce2, "UMAP")[, 2]),
  as.data.frame(colData(sce2)))

set.seed(11901)
sce2 <- runUMAP(sce2, dimred = "corrected", name = "UMAP_corrected")
corrected_umap <- cbind(
  data.frame(
    x = reducedDim(sce2, "UMAP_corrected")[, 1],
    y = reducedDim(sce2, "UMAP_corrected")[, 2]),
  as.data.frame(colData(sce2)))
```
...
mnn_out_1 <- fastMNN(
  multiBatchNorm(sce3, batch = sce3$batch),
  batch = sce3$batch,
  cos.norm = FALSE,
  d = ncol(reducedDim(sce3, "PCA")),
  auto.merge = TRUE,
  subset.row = hvg)
...
*
Among the three merge orders, i.e. *auto merge* (Table \@ref(tab:tab1-CL-removed)), *manual merge 1* (Table \@ref(tab:tab2-CL-removed)), and *manual merge 2* (Table \@ref(tab:tab3-CL-removed)), the *manual merge 2* lead to the least loss of biological variance from the dataset.
...
```{r}
reducedDim(sce3, "corrected_1") <- reducedDim(mnn_out_1, "corrected")
reducedDim(sce3, "corrected_2") <- reducedDim(mnn_out_2, "corrected")
reducedDim(sce3, "corrected_3") <- reducedDim(mnn_out_3, "corrected")

# generate UMAP
set.seed(1248)
sce3 <- runUMAP(sce3, dimred = "corrected_1", name = "UMAP_corrected_1")
corrected_umap_1 <- cbind(
  data.frame(
    x = reducedDim(sce3, "UMAP_corrected_1")[, 1],
    y = reducedDim(sce3, "UMAP_corrected_1")[, 2]),
  as.data.frame(colData(sce3)))

set.seed(1248)
sce3 <- runUMAP(sce3, dimred = "corrected_2", name = "UMAP_corrected_2")
corrected_umap_2 <- cbind(
  data.frame(
    x = reducedDim(sce3, "UMAP_corrected_2")[, 1],
    y = reducedDim(sce3, "UMAP_corrected_2")[, 2]),
  as.data.frame(colData(sce3)))

set.seed(1248)
sce3 <- runUMAP(sce3, dimred = "corrected_3", name = "UMAP_corrected_3")
corrected_umap_3 <- cbind(
  data.frame(
    x = reducedDim(sce3, "UMAP_corrected_3")[, 1],
    y = reducedDim(sce3, "UMAP_corrected_3")[, 2]),
  as.data.frame(colData(sce3)))

# re-clustering after each MNN correction
set.seed(4759)
snn_gr_1 <- buildSNNGraph(sce3, use.dimred = "corrected_1")
clusters_1 <- igraph::cluster_louvain(snn_gr_1)
sce3$cluster_1 <- factor(clusters_1$membership)
cluster_colours_1 <- setNames(
  scater:::.get_palette("tableau10medium")[seq_len(nlevels(sce3$cluster_1))],
  levels(sce3$cluster_1))
sce3$colours$cluster_colours_1 <- cluster_colours_1[sce3$cluster_1]

set.seed(4759)
snn_gr_2 <- buildSNNGraph(sce3, use.dimred = "corrected_2")
clusters_2 <- igraph::cluster_louvain(snn_gr_2)
sce3$cluster_2 <- factor(clusters_2$membership)
cluster_colours_2 <- setNames(
  scater:::.get_palette("tableau10medium")[seq_len(nlevels(sce3$cluster_2))],
  levels(sce3$cluster_2))
sce3$colours$cluster_colours_2 <- cluster_colours_2[sce3$cluster_2]

set.seed(4759)
snn_gr_3 <- buildSNNGraph(sce3, use.dimred = "corrected_3")
clusters_3 <- igraph::cluster_louvain(snn_gr_3)
sce3$cluster_3 <- factor(clusters_3$membership)
cluster_colours_3 <- setNames(
  scater:::.get_palette("tableau10medium")[seq_len(nlevels(sce3$cluster_3))],
  levels(sce3$cluster_3))
sce3$colours$cluster_colours_3 <- cluster_colours_3[sce3$cluster_3]
```

Figure \@ref(fig:batch-correction-test-CL-removed) shows an overview of comparisons between the unmerged and merged data  (from left to right column: *unmerged*, *auto merge*, *manual merge 1*, and *manual merge 2*) broken down by different experimental factors.
...
p1 <- plotReducedDim(sce3, "UMAP", colour_by = "plate_number", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = plate_number_colours, name = "plate_number")
p2 <- plotReducedDim(sce3, "UMAP_corrected_1", colour_by = "plate_number", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = plate_number_colours, name = "plate_number")
p3 <- plotReducedDim(sce3, "UMAP_corrected_2", colour_by = "plate_number", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = plate_number_colours, name = "plate_number")
p4 <- plotReducedDim(sce3, "UMAP_corrected_3", colour_by = "plate_number", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = plate_number_colours, name = "plate_number")

p5 <- plotReducedDim(sce3, "UMAP", colour_by = "sample_name", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_name_colours, name = "sample_name")
p6 <- plotReducedDim(sce3, "UMAP_corrected_1", colour_by = "sample_name", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_name_colours, name = "sample_name")
p7 <- plotReducedDim(sce3, "UMAP_corrected_2", colour_by = "sample_name", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_name_colours, name = "sample_name")
p8 <- plotReducedDim(sce3, "UMAP_corrected_3", colour_by = "sample_name", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_name_colours, name = "sample_name")

p9 <- plotReducedDim(sce3, "UMAP", colour_by = "cluster", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = cluster_colours, name = "cluster")
p10 <- plotReducedDim(sce3, "UMAP_corrected_1", colour_by = "cluster_1", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = cluster_colours_1, name = "cluster_1")
p11 <- plotReducedDim(sce3, "UMAP_corrected_2", colour_by = "cluster_2", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = cluster_colours_2, name = "cluster_2")
p12 <- plotReducedDim(sce3, "UMAP_corrected_3", colour_by = "cluster_3", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = cluster_colours_3, name = "cluster_3")

p13 <- plotReducedDim(sce3, "UMAP", colour_by = "post_hoc_sample_gate", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_gate_colours, name = "post_hoc_sample_gate")
p14 <- plotReducedDim(sce3, "UMAP_corrected_1", colour_by = "post_hoc_sample_gate", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_gate_colours, name = "post_hoc_sample_gate")
p15 <- plotReducedDim(sce3, "UMAP_corrected_2", colour_by = "post_hoc_sample_gate", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_gate_colours, name = "post_hoc_sample_gate")
p16 <- plotReducedDim(sce3, "UMAP_corrected_3", colour_by = "post_hoc_sample_gate", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = sample_gate_colours, name = "post_hoc_sample_gate")

p17 <- plotReducedDim(sce3, "UMAP", colour_by = "tissue", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = tissue_colours, name = "tissue")
p18 <- plotReducedDim(sce3, "UMAP_corrected_1", colour_by = "tissue", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = tissue_colours, name = "tissue")
p19 <- plotReducedDim(sce3, "UMAP_corrected_2", colour_by = "tissue", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = tissue_colours, name = "tissue")
p20 <- plotReducedDim(sce3, "UMAP_corrected_3", colour_by = "tissue", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = tissue_colours, name = "tissue")

p21 <- plotReducedDim(sce3, "UMAP", colour_by = "donor", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = donor_colours, name = "donor")
p22 <- plotReducedDim(sce3, "UMAP_corrected_1", colour_by = "donor", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = donor_colours, name = "donor")
p23 <- plotReducedDim(sce3, "UMAP_corrected_2", colour_by = "donor", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = donor_colours, name = "donor")
p24 <- plotReducedDim(sce3, "UMAP_corrected_3", colour_by = "donor", theme_size = 7, point_size = 0.2) + 
  scale_colour_manual(values = donor_colours, name = "donor")
...
To get an insight, here we break down the UMAP plot above further by `plate_number` (Figure \@ref(fig:umap-plate-number-CL-removed)), `post_hoc_sample_gate` (Figure \@ref(fig:umap-post-hoc-sample-gate-CL-removed)), and `tissue` (Figure \@ref(fig:umap-tissue-CL-removed)).
...
From the perspective of preserving the biological-feature-of-interest, after the batch correction, we can still observe: (i) the "shifting" of the transcriptomic stages of cells `P6` to `P7`, then `P8` (Figure \@ref(fig:umap-post-hoc-sample-gate-CL-removed)); (ii) `Blood` contains cells mostly at `P8` only, whilst `Thymus` contains cells at all three phases, i.e. `P6`, `P7` and `P8` (Figure \@ref(fig:umap-tissue-CL-removed)).
...
```{r umap-tissue-CL-removed, fig.cap = "UMAP plot of the dataset. Each point represents a cell and each panel highlights cells from a particular `tissue`when data is unmerged (left) and merged by manual merge 2 (right).", fig.asp = 2/3, layout="l-body"}
umap_df <- makePerCellDF(sce3)
...
Taken altogether, we determine *manual merge 2* as the best possible merge order for data integration of this dataset and proceed.

```{r}
reducedDim(sce, "corrected") <- reducedDim(mnn_out_3, "corrected")
```
...
To sumup, there are `r nlevels(sce$cluster)` clusters detected, shown on the UMAP plot Figure \@ref(fig:clusterplot-umap-CL-removed-merged) and broken down by experimental factors in Figure \@ref(fig:cluster-barplot-CL-removed-merged).

```{r clusterplot-umap-CL-removed-merged, fig.cap = "UMAP plot, where each point represents a droplet and is coloured according to the legend.", fig.asp = 1.25}
p1 <- ggcells(sce, aes(x = UMAP.1, y = UMAP.2)) +
  geom_point(aes(colour = cluster), size = 0.25) +
  scale_colour_manual(values = cluster_colours) + 
  theme_cowplot(font_size = 8) + 
...
```{r cluster-barplot-CL-removed-merged, fig.asp = 1, fig.cap = "Breakdown of clusters by experimental factors."}
p1 <- ggcells(sce) + 
  geom_bar(
    aes(x = cluster, fill = sample_type),
    position = position_fill(reverse = TRUE)) +
