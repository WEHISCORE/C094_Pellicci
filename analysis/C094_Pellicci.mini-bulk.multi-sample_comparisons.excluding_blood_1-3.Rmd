...
genes <- c(
  ...
  # Chemokines
  "CCR5", "CCR9", "CXCR3", "CXCR6", "CCR6" = "CD196",
  # Surface molecules
...
genes_df <- data.frame(
  ...
  Detected = genes %in% rownames(y),
  check.names = FALSE)
knitr::kable(
  genes_df,
  caption = "Genes requested by Dan for violin plots, their corresponding name in the dataset, and whether they are sufficiently expressed for DE analysis (`Detected`).")
```
...
scater::plotExpression(
  se,
  features = intersect(genes_df$`Gene name in dataset`, rownames(y)),
  x = "group",
  exprs_values = "logCPM",
  scales = "free_y",
  ncol = 4,
...
for (i in seq_len(nrow(genes_df))) {
  g <- intersect(genes_df$`Gene name in dataset`[i], rownames(y))
  if (length(g)) {
