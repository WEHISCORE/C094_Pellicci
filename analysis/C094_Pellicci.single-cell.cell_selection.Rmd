...
We would build the graph using the *principal components (PCA)*.
...
group_colours <- setNames(
  palette.colors(nlevels(sce$group), "Paired"),
  levels(sce$group))
...
**NOTE** for the unmerged data based on these figures:
  ...
  - most of the cells in `S1` and `S2` form one single cluster, i.e. cluster `2`, and they found mostly in `Thymus` sample only
...
Further, we also look for genes that are specifically upregulated in cluster `5` compared to all other clusters, so as to help determine if `intermediate monocyte` is the  exact identity of this cluster.
...
plotHeatmap(
    ...
    center = TRUE,
    zlim = c(-3, 3),
    order_columns_by = c("cluster", "group", "sample", "stage", "plate_number", "tissue", "donor"),
    cluster_rows = TRUE,
    fontsize = 5,
    column_annotation_colors = list(
      cluster = cluster_colours,
...
Again, the `SKW3` cell line also seems to affect the grouping of some of the sample cells (where cells of cell line were grouped with those from the `Thymus` cells and form a standalone cluster.
...
Also, after our online discussion with Dan and Stuart on 30 June 2021, we confirmed that cells in cluster `5` are contaminants.
...
## Investigation of cluster `4` (after merging)
...
Figure \@ref(fig:batch-correction-vs) shows an overview of comparisons between the unmerged and merged data broken down by different experimental factors.
After the MNN correction, we obtained *four* different clusters, in which, the cluster `4` drawn our particular attention.
...
p3 <- plotReducedDim(sce, "UMAP", colour_by = "cluster", theme_size = 7, point_size = 0.2) + 
...
If we look into the distribution of the cluster `4` cells on each plate (Figure \@ref(fig:cluster-4-vs-layout)), there is perhaps a bias towards cells being in cluster `4` when they are near the edges of the plate, specifically towards columns 1-3 and in particular on plate LCE514.

```{r cluster-4-vs-layout, fig.cap = "Distribution of cluster 4 cells against well position of each plate.", fig.asp = 3/4}
...
In Figure \@ref(fig:cluster-4-vs-column), *blue* line is the median proportion and *red* line is median of cluster `4` cell on that plate.
We can see that, on the whole, LCE508 and LCE514 have the most cluster `4` cells.
In which, plate LCE508 has more cluster 4 cells if considering all columns, whilst columns 1-3 on plate LCE514 have way more cluster 4 cells on average.
We speculate that this type of bias could arise due to issues with plate alignment whereby the columns near the edge do not quite get the cell in the middle of the well and so we get less RNA from these libraries.

```{r cluster-4-vs-column, fig.cap = "Distribution of cluster 4 cells by column of each plate. Blue and red line indicate the median proportion and median of cluster `4` cell on that plate, respectively.", fig.asp = 3/4}
p <- proportions(table(sce$cluster, factor(substr(sce$well_position, 2, 3), 1:24), sce$plate_number), 2:3)

par(mfrow = c(2, 3))
invisible(
  lapply(levels(sce$plate_number), function(k) {
  plot(p[4, , k], ylim = c(0, 1), type = "b", main = k, ylab = "prop. cluster 4", xlab = "column")
  abline(h = median(p[4, , ]), col = "blue")
  abline(h = median(p[4, , k]), col = "red")
})
)
```

Together with the fact that the cells from cluster `4` tend to have smaller library size (Figure \@ref(fig:qcplot-by-cluster-donor-linear)) and marker gene with undetermined cell-type (Figure \@ref(fig:heat-uniquely-up-logExp-cluster-4)), after our discussion with Dan and Louis on 10 Aug 2021 online, we decide to exclude cells cluster `4` from the dataset for clarity of the results.
...
```{r}
# removal of cluster 4 cells from SCE
sce <- sce[ ,!(sce$cluster=="4")]
colData(sce) <- droplevels(colData(sce))
```
...
The processed *SingleCellExperiment* object is available (see [`data/SCEs/C094_Pellicci.single-cell.cell_selected.SCE.rds`](../data/SCEs/C094_Pellicci.single-cell.cell_selected.SCE.rds)).
