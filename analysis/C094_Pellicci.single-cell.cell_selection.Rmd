...
```{r batch-correction-test-cell-selected, fig.asp = 1.5, fig.cap = "Comparison between batch-uncorrected data (left column) and -corrected data by manual merge orders (right column)."}
...
p13 <- plotReducedDim(sce, "UMAP", colour_by = "group") + 
  scale_colour_manual(values = group_colours, name = "group")
p14 <- plotReducedDim(sce, "UMAP_corrected", colour_by = "group") + 
  scale_colour_manual(values = group_colours, name = "group")

p1 + p2 + p3 + p4 +
  p5 + p6 + p7 + p8 +
  p9 + p10 + p11 + p12 +
  p13 + p14 + plot_spacer() + plot_spacer() +
