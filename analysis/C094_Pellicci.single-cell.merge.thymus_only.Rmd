...
*NOTE* for the unmerged data:
  - there are two big populations, which subdivided into two and three clusters, respectively;
  - cells in cluster `1` and `2` are mostly in `S1` and `S2`;
  - cells in cluster `3`, `4` and `5` are mostly in `S3`;
...
To get an insight, here we focus on the uncorrected and the corrected data by the *manual merge*, then break down the UMAP plot above further by `plate_number` (Figure \@ref(fig:umap-plate-number-vs)) and `stage` (Figure \@ref(fig:umap-stage-vs)).
...
From the perspective of preserving the biological-feature-of-interest, after the batch correction, we can still observe the "shifting" of the transcriptomic stages of cells `P6` to `P7`, then `P8` (Figure \@ref(fig:umap-stage-vs)).
...
```{r umap-stage-vs, fig.cap = "UMAP plot of the dataset. Each point represents a cell and each panel highlights cells from a particular `stage` when data is unmerged (left) and merged by manual merge 2 (right).", fig.asp = 3/4, layout="l-body"}
...
saveRDS(
  sce,
  here("data", "SCEs", "C094_Pellicci.single-cell.merged.thymus_only.SCE.rds"),
  compress = "xz")
```

The processed *SingleCellExperiment* object is available (see [`data/SCEs/C094_Pellicci.single-cell.merged.thymus_only.SCE.rds`](../data/SCEs/C094_Pellicci.single-cell.merged.thymus_only.SCE.rds)).
