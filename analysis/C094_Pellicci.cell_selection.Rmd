...
Notably, the SKW3 cell line seems to affect the grouping of some of the sample cells (where cells of cell line were grouped with those from the thymus cell and form a standalone cluster in Figure \@ref(fig:umap-tissue)).
To identify the identity of this cells, we perform cell annotation at cluster level with SingleR.
...
**TODO: based on the SingleR annotation, cluster 5 is potentially be a contaminant (intermediate monocytes and is not our interested gamma-delta T cells) that sneaked through the FACS gate and could considered to be removed**
